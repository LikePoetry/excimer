...
#define THREADS 8
        ...
        static void CacheFontAtlas(const std::string& fontName, float fontSize, AtlasHeader header, const void* pixels)
        {
            ...
            {
                stream.close();
                EXCIMER_LOG_ERROR("Failed to cache font atlas to {0}", filepath.string());
                return;
            }
        ...
        void Font::Init()
        {
            ...
            if (m_FontDataSize == 0)
            {
                ...
                EXCIMER_LOG_INFO("Font: Loading Font {0}", m_FilePath);
                fontInput.fontFilename = outPath.c_str();
                m_FilePath = outPath;

                if (!font.load(fontInput.fontFilename))
                {
                    EXCIMER_LOG_ERROR("Font: Failed to load font! - {0}", fontInput.fontFilename);
                    return;
                }
            }
            else
            {
                ...
                {
                    EXCIMER_LOG_ERROR("Font: Failed to load font from data!");
                    return;
                }
            }
            ...
            EXCIMER_ASSERT(glyphsLoaded >= 0, "");
            EXCIMER_LOG_TRACE("Font: Loaded geometry of {0} out of {1} glyphs", glyphsLoaded, (int)charset.size());
            // List missing glyphs
            if (glyphsLoaded < (int)charset.size())
            {
                EXCIMER_LOG_WARN("Font: Missing {0} {1}", (int)charset.size() - glyphsLoaded, fontInput.glyphIdentifierType == GlyphIdentifierType::UNICODE_CODEPOINT ? "codepoints" : "glyphs");
            }
            ...
            {
                EXCIMER_LOG_ERROR("Font: Could not fit {0} out of {1} glyphs into the atlas.", remaining, (int)m_MSDFData->Glyphs.size());
            }
            atlasPacker.getDimensions(config.width, config.height);
            EXCIMER_CORE_ASSERT(config.width > 0 && config.height > 0, "");
            config.emSize = atlasPacker.getScale();
            config.pxRange = atlasPacker.getPixelRange();
            if (!fixedScale)
                EXCIMER_LOG_TRACE("Font: Glyph size: {0} pixels/EM", config.emSize);
            if (!fixedDimensions)
                EXCIMER_LOG_TRACE("Font: Atlas dimensions: {0} x {1}", config.width, config.height);
