...
namespace Excimer
{
    ...
    Camera::Camera(float aspectRatio, float nearS, float farS)
        : m_AspectRatio(aspectRatio)
        , m_Scale(1.0f)
        , m_FrustumDirty(true)
        , m_ProjectionDirty(true)
    ...
    Maths::Ray Camera::GetScreenRay(float x, float y, const glm::mat4& viewMatrix, bool flipY) const
    {
        ...
        glm::vec4 nearS = viewProjInverse * glm::vec4(x, y, 0.0f, 1.0f);
        nearS /= nearS.w;

        glm::vec4 farS = viewProjInverse * glm::vec4(x, y, 1.0f, 1.0f);
        farS /= farS.w;

        ret.Origin = glm::vec3(nearS);
        ret.Direction = glm::normalize(glm::vec3(farS) - ret.Origin);
