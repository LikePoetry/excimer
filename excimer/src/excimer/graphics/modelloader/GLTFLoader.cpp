...
namespace Excimer::Graphics
{
    ...
    std::vector<Graphics::Mesh*> LoadMesh(tinygltf::Model& model, tinygltf::Mesh& mesh, std::vector<SharedPtr<Material>>& materials, Maths::Transform& parentTransform)
    {
        ...
        for (auto& primitive : mesh.primitives)
        {
            ...
            // -------- Indices ----------
            {
                ...
                {
                    EXCIMER_LOG_WARN("Unsupported indices data type - {0}", componentTypeByteSize);
                }
            }
    ...
    void LoadNode(Model* mainModel, int nodeIndex, const glm::mat4& parentTransform, tinygltf::Model& model, std::vector<SharedPtr<Material>>& materials, std::vector<std::vector<Graphics::Mesh*>>& meshes)
    {
        ...
        EXCIMER_LOG_INFO("asset.copyright : {0}", model.asset.copyright);
        EXCIMER_LOG_INFO("asset.generator : {0}", model.asset.generator);
        EXCIMER_LOG_INFO("asset.version : {0}", model.asset.version);
        EXCIMER_LOG_INFO("asset.minVersion : {0}", model.asset.minVersion);
#endif
    ...
    void Model::LoadGLTF(const std::string& path)
    {
        ...
        {
            EXCIMER_PROFILE_SCOPE(".glb binary loading");
            ret = tinygltf::TinyGLTF().LoadBinaryFromFile(&model, &err, &warn, path);
        }
        else // assume ascii glTF.
        {
            EXCIMER_PROFILE_SCOPE(".gltf loading");
            ret = tinygltf::TinyGLTF().LoadASCIIFromFile(&model, &err, &warn, path);
        }

        if (!err.empty())
        {
            EXCIMER_LOG_ERROR(err);
        }

        if (!warn.empty())
        {
            EXCIMER_LOG_ERROR(warn);
        }

        if (!ret)
        {
            EXCIMER_LOG_ERROR("Failed to parse glTF");
        }
        {
            EXCIMER_PROFILE_SCOPE("Parse GLTF Model");
            ...
            const tinygltf::Scene& gltfScene = model.scenes[Excimer::Maths::Max(0, model.defaultScene)];
