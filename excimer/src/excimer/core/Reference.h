...
namespace Excimer
{
    ...
    private:
        ...
        {
            EXCIMER_ASSERT(ptr, "Creating shared ptr with nullptr");
    ...
    template <class T>
    class EXCIMER_EXPORT WeakReference
    {
    public:
        ...
        explicit WeakReference(T* ptr) noexcept
            : m_Ptr(ptr)
        {
            EXCIMER_ASSERT(ptr, "Creating weak ptr with nullptr");
...
{
    template <typename T>
    struct hash<Excimer::Reference<T>>
    {
        size_t operator()(const Excimer::Reference<T>& x) const
        {
            return hash<T*>()(x.get());
        }
    };
