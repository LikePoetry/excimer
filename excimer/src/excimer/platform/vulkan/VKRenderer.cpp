...
namespace Excimer
{
	namespace Graphics
	{
        ...
        void VKRenderer::InitInternal()
        {
            ...
            // Pool
            // TODO Set 100000 as pool sizes;
            //VK_CHECK_RESULT(vkCreateDescriptorPool(VKDevice::Get().GetDevice(), &pool_info, nullptr, &s_DescriptorPool));

            VK_CHECK_RESULT(vkCreateDescriptorPool(VKDevice::Get().GetDevice(), &poolCreateInfo, nullptr, &s_DescriptorPool));
        ...
        void VKRenderer::BindDescriptorSetsInternal(Graphics::Pipeline* pipeline, Graphics::CommandBuffer* commandBuffer, uint32_t dynamicOffset, Graphics::DescriptorSet** descriptorSets, uint32_t descriptorCount)
        {
            ...
            for (uint32_t i = 0; i < descriptorCount; i++)
            {
                if (descriptorSets[i])
                {
                    ...
                    EXCIMER_ASSERT(vkDesSet->GetHasUpdated(Renderer::GetMainSwapChain()->GetCurrentBufferIndex()), "Descriptor Set has not been updated before");
                    numDesciptorSets++;
                }
            }
