...
namespace Excimer
{
	namespace Graphics
	{
		...
		bool VKSwapChain::Init(bool vsync)
		{
			...
			if(!m_Surface)
			{
				EXCIMER_LOG_CRITICAL("[VULKAN] Failed to create window surface!");
			}
			...
			if (queueIndexSupported == VK_FALSE)
				EXCIMER_LOG_ERROR("Present Queue not supported!");
		...
		void VKSwapChain::AcquireNextImage()
		{
			...
			{
				EXCIMER_PROFILE_SCOPE("vkAcquireNextImageKHR");
				...
				{
					EXCIMER_LOG_INFO("Acquire Image result : {0}", result == VK_ERROR_OUT_OF_DATE_KHR ? "Out of Date" : "SubOptimal");
				...
				{
					EXCIMER_LOG_CRITICAL("[VULKAN] Failed to acquire swap chain image!");
				}
