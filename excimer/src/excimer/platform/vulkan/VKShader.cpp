...
namespace Excimer
{
    namespace Graphics
    {
        ...
        VkFormat GetVulkanFormat(const spirv_cross::SPIRType& type)
        {
            ...
            {
                EXCIMER_LOG_WARN("Tried to convert a structure or SPIR sampler into a VkFormat enum value!");
            ...
            else if (type.basetype == SPIRType::Image || type.basetype == SPIRType::SampledImage)
            {
                ...
                default:
                    EXCIMER_LOG_WARN("Failed to convert an image format to a VkFormat enum.");
                    return VK_FORMAT_UNDEFINED;
                }
            }
            else if (type.vecsize == 1)
            {
                ...
                {
                    EXCIMER_LOG_WARN("Invalid type width for conversion of SPIR-Type to VkFormat enum value!");
                    return VK_FORMAT_UNDEFINED;
                }
            }
            ...
            else if (type.vecsize == 4)
            {
                ...
                {
                    EXCIMER_LOG_WARN("Invalid type width for conversion to a VkFormat enum");
                    return VK_FORMAT_UNDEFINED;
                }
            }
            else
            {
                EXCIMER_LOG_WARN("Vector size in vertex input attributes isn't explicitly supported for parsing from SPIRType->VkFormat");
                return VK_FORMAT_UNDEFINED;
            }
        }
        ...
        uint32_t GetStrideFromVulkanFormat(VkFormat format)
        {
            ...
            default:
                EXCIMER_LOG_ERROR("Unsupported Format {0}", format);
                return 0;
            }
        ...
        bool VKShader::Init()
        {
            ...
            EXCIMER_LOG_INFO("Loading Shader : {0}", m_Name);
            ...
            if (files.empty())
                EXCIMER_LOG_ERROR("Failed to load shader {0}", m_Name);
        ...
        void VKShader::LoadFromData(const uint32_t* source, uint32_t fileSize, ShaderType shaderType, int currentShaderStage)
        {
            ...
            for (auto& u : resources.uniform_buffers)
            {
                ...
                SHADER_LOG(EXCIMER_LOG_INFO("Found UBO {0} at set = {1}, binding = {2}", u.name.c_str(), set, binding));
                ...
                for (int i = 0; i < memberCount; i++)
                {
                    ...
                    SHADER_LOG(EXCIMER_LOG_INFO("{0} - Size {1}, offset {2}", uniformName, size, offset));
                }
            }
            ...
            for (auto& u : resources.push_constant_buffers)
            {
                ...
                {
                    SHADER_LOG(EXCIMER_LOG_INFO("Accessing Member {0} offset {1}, size {2}", range.index, range.offset, range.range));
                    size += uint32_t(range.range);
                }

                SHADER_LOG(EXCIMER_LOG_INFO("Found Push Constant {0} at set = {1}, binding = {2}", u.name.c_str(), set, binding, type.array.size() ? uint32_t(type.array[0]) : 1));
                ...
                for (int i = 0; i < memberCount; i++)
                {
                    ...
                    member.size = (uint32_t)size;
                    member.offset = offset;
                    member.type = SPIRVTypeToSlightDataType(type);
            ...
            for (auto& u : resources.sampled_images)
            {
                ...
                auto& type = comp.get_type(u.type_id);
                SHADER_LOG(EXCIMER_LOG_INFO("Found Sampled Image {0} at set = {1}, binding = {2}", u.name.c_str(), set, binding));
            ...
            for (auto& u : resources.storage_images)
            {
                ...
                SHADER_LOG(EXCIMER_LOG_INFO("Found Storage Image {0} at set = {1}, binding = {2}", u.name.c_str(), descriptorSet, binding));
        ...
        Shader* VKShader::CreateFuncVulkan(const std::string& filepath)
        {
            ...
            Excimer::VFS::Get().ResolvePhysicalPath(filepath, physicalPath, false);
