...
namespace Excimer
{
    void EmbedTexture(const std::string& texFilePath, const std::string& outPath, const std::string& arrayName)
    {
        ...
        std::ofstream file;
        file.open(outPath);
        file << "//Generated by Slight using " << texFilePath << std::endl;
    ...
    void EmbedShader(const std::string& shaderPath)
    {
        ...
        if (!data)
        {
            EXCIMER_LOG_WARN("Failed to load shader : {0}", shaderPath);
            return;
        }
        ...
        std::stringstream ss;
        ss << "// Header generated by Slight Editor\n\n";
        ss << "#include <array>\n";
        ss << "#include <cstdint>\n\n";
        ...
        EXCIMER_LOG_INFO("Embedded Shader {0}", folder + "Headers/" + shaderName + ".hpp");
    }
}
