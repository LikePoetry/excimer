...
//#include <excimer/Scripting/Lua/LuaScriptComponent.h>
...
namespace Excimer
{
	...
	void HierarchyPanel::DrawNode(entt::entity node, entt::registry& registry)
	{
        ...
        if (show)
        {
            ...
            if (payload != NULL && payload->IsDataType("Drag_Entity"))
            {
                ...
                EXCIMER_ASSERT(payload->DataSize == sizeof(entt::entity*), "Error ImGUI drag entity");
    ...
    void HierarchyPanel::OnImGui()
    {
        ...
        m_SelectUp = Input::Get().GetKeyPressed(Excimer::InputCode::Key::Up);
        m_SelectDown = Input::Get().GetKeyPressed(Excimer::InputCode::Key::Down);
